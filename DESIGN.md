...
## `keppy/viz/vtk_viewer.py` — `VTKOrbitViewer`

### Goal

The original keplerpp viewer uses Mayavi/VTK and permits unrestricted
trackball-camera movement.  `VTKOrbitViewer` preserves that interaction in
native VTK: it renders the simulation in three dimensions, uses a black
background by default, and uses `vtkInteractorStyleTrackballCamera` for
free camera rotation.

### Interaction and animation

The VTK viewer is optional (`pip install "keppy[vtk]"`) and powers the CLI
`--plot` flag. Left-drag rotates the camera and the mouse wheel zooms. Space
pauses/resumes animation; right-clicking a body pauses and centres the
reference frame on it. Press `0` to return to the absolute frame, `1`–`9` to
centre a body by its displayed order, `n` to toggle labels, and `o` to toggle
trails.

Positions are pre-computed in a `(n_steps, n_bodies, 3)` array. Each VTK
timer event updates sphere actors, billboard labels, and polyline trails for
the next frame. Recentring subtracts the selected body's position from every
body, then makes the selected body's new origin the active camera focal point
while preserving the user's current view direction and zoom.

### Trail control

`trail_length=None` shows the complete history. An integer limits each VTK
polyline to `slice(max(0, frame_index - trail_length), frame_index + 1)`.
The slice is applied after the reference-frame transformation, so trails
remain correct after recentering.
